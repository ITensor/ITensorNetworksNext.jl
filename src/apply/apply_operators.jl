...
# bipartition, so diagonalize it in the transposed (bra, ket) one. We use the balanced gauge
# `v * √d * v'` (and inverse `v * √d⁻¹ * v'`), which works with fermions; the asymmetric gauge
# `√d * v'` has an issue that is under investigation.
