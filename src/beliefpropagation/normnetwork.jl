...
names (the link axes from `tn`) and the same fresh `uniquename`-generated bra-side names,
...
Each ket link axis on every edge is paired with a fresh `uniquename`-generated bra link
...
function normnetwork(tn)
    linknames_map = Dict(
        e => Dict(n => uniquename(n) for n in linknames(tn, e))
            for e in edges(tn)
    )
